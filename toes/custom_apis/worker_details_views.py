...
#display workers of without smaertphones
@api_view(['GET'])
@permission_classes([IsAuthenticated])
def display_all_withoutsmartphone(request):
    cursor=connection.cursor()
    #cursor.execute(f"select * from authapp_worker_Details")
    cursor.execute('select first_name, phone, city, category_1, category_1_vc, category_1_exp, category_2, category_2_vc, category_2_exp, category_3, category_3_vc, category_3_exp,smartphone  from authapp_user INNER JOIN authapp_workerdetails ON authapp_user.id = authapp_Workerdetails.user_id')
